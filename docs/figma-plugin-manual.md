...
_图 1：整体管线架构 — 从 Figma 设计文件到前端应用的完整数据流_
...
每个课文一个独立的 Figma 文件，**同时包含 Export Assets 和 文字资源\_ Frame**。
...
- 课文文件：扫描 Export Assets + 文字资源\_ → 上传到对应路径
...
- **项目名称**: 文言文预习
- **Figma 设计稿 URL**: https://www.figma.com/design/7vHDlwr34hvSyGv5oIiOZO/%E6%96%87%E8%A8%80%E6%96%87%E9%A2%84%E4%B9%A0?node-id=0-1&t=5f6RzZ4BIqWODWKr-1
- **fileKey**: 7vHDlwr34hvSyGv5oIiOZO
- **画布尺寸**: 3040×1710
- **字体**: Source Han Serif CN（思源宋体）
- **主色**: #851E0E（朱红）、#687857（橄榄绿）、#FFFFFF（白色）

### 11.2 设计稿用途

| 用途             | 说明                                                                   |
| ---------------- | ---------------------------------------------------------------------- |
| 前端 UI 实现参考 | 页面布局、配色、字体、间距等视觉参数                                   |
| 插件样式参数提取 | 使用 `scanAllStyles` 提取颜色、字体、边框、圆角、阴影等属性            |
| 资源导出         | 从 `Export Assets` Frame 导出图片资源，`文字资源_` Frame 导出文字 JSON |

### 11.3 设计稿与插件的关系

- 插件 `scanAllStyles` 函数从当前页面所有节点提取视觉属性，生成 `styles/{Frame名}.json` 文件
- 提取的参数包括：颜色（SOLID 类型 RGB→HEX）、字体（family/size/weight/lineHeight）、边框（width/align）、圆角、尺寸/位置、阴影/模糊效果、自动布局属性、文本对齐/装饰/段落间距等
- 前端通过 `docs/figma-reference.md` 中的设计 token 映射到 CSS 变量

## 修订记录

| 日期       | 版本  | 变更内容                                        |
| ---------- | ----- | ----------------------------------------------- |
| 2026-08-17 | 1.1.0 | 新增 Figma 设计稿参考附录，补充样式参数提取说明 |
