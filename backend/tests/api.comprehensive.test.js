...
describe('登录接口', () => {
  describe('POST /api/auth/login', () => {
    beforeAll(async () => {
      ...
      await request(app)
        .post('/api/students')
        .send({ studentId: '30001', name: '孙七' });
    });

    it('已注册学生应登录成功并返回token', async () => {
      const res = await request(app)
        .post('/api/auth/login')
        .send({ student_id: '30001' });
  ...
  describe('POST /api/submit', () => {
    ...
    it('极大JSON请求体应被限制', async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        largeData.answers[`Q_large_${i}`] = 1;
        largeData.questions.push({ id: `Q_large_${i}`, correctAnswer: 1 });
      }
