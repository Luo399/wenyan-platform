...
// ============================================================
// localStorage 答题记录持久化（供 AdaptQuiz / Level1Quiz 使用）
// ============================================================
...
export interface QuizQuestionRecord {
  ...
  userAnswer: string | number | (string | number)[] | null | undefined
...
/** 生成 localStorage 存储键 */
function getStorageKey(studentId: string): string | null {
  if (!studentId) return null
  return `wenyan_quiz_records_${studentId}`
}
...
export function loadQuizRecords(studentId: string): QuizRecordReport[] {
  ...
  try {
    ...
    if (!raw) return []
    return JSON.parse(raw) as QuizRecordReport[]
  } catch (e) {
    debugWarn('[useQuizProgress] 读取 localStorage 失败:', e)
    return []
  }
}
